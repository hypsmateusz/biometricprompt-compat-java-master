...
public class Encryption extends AppCompatActivity {
     ...
     *
     * KEY AND SALT generating random string for encryption function
     */
     ...
     */
    public static Encryption getDefault(String key, String salt, String iv) {
        try {
            return Builder.getDefaultBuilder(key,salt,Base64.decode(iv,Base64.DEFAULT)).build();
    ...
    *Function generating random String,salt,iv

     */

    private static String generateString(){
        int leftLimit = 48; // first sign
        int rightLimit = 122; // last sign
    ...
    public String encrypt(String data) throws UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidAlgorithmParameterException, InvalidKeyException, InvalidKeySpecException, BadPaddingException, IllegalBlockSizeException {
        ...
        String key = mBuilder.getKey();
        byte[] iv = mBuilder.getIv();
        SecretKey secretKey = getSecretKey(hashTheKey(key));
        byte[] dataBytes = data.getBytes(mBuilder.getCharsetName());
        Cipher cipher = Cipher.getInstance(mBuilder.getAlgorithm());
        IvParameterSpec ivSpec = new IvParameterSpec(iv);
        cipher.init(Cipher.ENCRYPT_MODE, secretKey, ivSpec);
        String pom = Base64.encodeToString(cipher.doFinal(dataBytes), mBuilder.getBase64Mode());
